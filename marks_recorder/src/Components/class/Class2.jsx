...
import { clas, filterClass2, deleteStudent } from '../../Redux/Actions';
...
class Class2 extends Component {
    ...
    render() {
        const { c_data, match, filterClass2, fil2, filteredData_c2, deleteStudent } = this.props
        ...
        if (!fil2) {
            return (
                <main>
                    ...
                    <Box display="flex" justifyContent="center" m={1} p={1} bgcolor="background.paper">
                        <Button
                            onClick={() => { filterClass2(this.state) }}
                            variant="contained"
                            color="secondary">
                            Apply
                            </Button>
                ...
                <main>
                    ...
                    <TableContainer component={Paper}>
                        <Table ria-label="caption table">
                            ...
                            <TableBody>
                                {filteredData_c2 && filteredData_c2.map((row) => (
                                    <TableRow hover key={uuidv4()} style={{ backgroundColor: row.grade === 'FAIL' ? '#c5cae9' : 'none' }}>
...
const mapStateToProps = state => {
    console.log('all students data', state.filteredData_c2)
    return {
        c_data: state.c_data,
        fil2: state.fil2,
        filteredData_c2: state.filteredData_c2.data
    };
};
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        filterClass2: a => dispatch(filterClass2(a)),
