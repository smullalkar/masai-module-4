...
import { clas, filterClass8, deleteStudent } from '../../Redux/Actions';
...
class Class8 extends Component {
    ...
    render() {
        const { c_data, match, filterClass8, fil8, filteredData_c8, deleteStudent } = this.props
        ...
        if (!fil8) {
            return (
                <main>
                    ...
                    <Box display="flex" justifyContent="center" m={1} p={1} bgcolor="background.paper">
                        <Button
                            onClick={() => { filterClass8(this.state) }}
                            variant="contained"
                            color="secondary">
                            Apply
                            </Button>
                ...
                <main>
                    ...
                    <TableContainer component={Paper}>
                        <Table ria-label="caption table">
                            ...
                            <TableBody>
                                {filteredData_c8 && filteredData_c8.map((row) => (
                                    <TableRow hover key={uuidv4()} style={{ backgroundColor: row.grade === 'FAIL' ? '#c5cae9' : 'none' }}>
...
const mapStateToProps = state => {
    console.log('all students data', state.filteredData_c8)
    return {
        c_data: state.c_data,
        fil8: state.fil8,
        filteredData_c8: state.filteredData_c8.data
    };
};
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        filterClass8: a => dispatch(filterClass8(a)),
